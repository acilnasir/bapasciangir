...
export default function LitmasClient() {
  const [keyword, setKeyword] = useState("");
  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState<Litmas[]>([]);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/litmas/search?nama=${encodeURIComponent(keyword)}`,
      );
      ...
      <section className="bg-primary py-20 text-center text-white">
        <div className="mx-auto max-w-4xl px-4">
          <h1 className="text-4xl font-bold md:text-5xl">
            Pencarian Data Litmas Dewasa
          </h1>

          <p className="mt-4 text-lg text-blue-100">
            Cari data klien berdasarkan nama untuk memantau status Litmas.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12">
        ...
        <div className="space-y-6">
          {results.map((item, index) => {
            const selesai = Boolean(item["TGL PENYELESAIAN"]?.trim());

            return (
              <div
                key={index}
                className="rounded-2xl border-l-4 border-l-tertiary bg-white p-8 shadow-md"
              >
                ...
                <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{item["NAMA PK"] || "-"}</p>
                  </div>
                ...
                <div className="mt-6 border-t pt-4 text-sm text-gray-500">
                  Data ini berasal dari E-Distribusi Litmas Bapas Purwokerto.
                </div>
              </div>
            );
          })}
