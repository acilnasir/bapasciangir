...
export default function DiklatSection({ profilId, initialData }: Props) {
  const [data, setData] = useState<Diklat[]>(initialData);
  ...
  function mulaiEdit(item: Diklat) {
    setEditId(item.id);
    setNama(item.nama);
    setTahun(item.tahun);
  }
  ...
  async function simpanData() {
    ...
    try {
      ...
      if (editId) {
        const res = await fetch(`/api/diklat/${editId}`, {
          method: "PUT",
          headers: {
  ...
  async function hapusData(id: string) {
    const confirmDelete = confirm("Yakin ingin menghapus Riwayat diklat ini?");

    if (!confirmDelete) return;

    try {
      const res = await fetch(`/api/diklat/${id}`, {
        method: "DELETE",
      });
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-neutral">
              <th className="p-2 text-left">Diklat dan Kompetensi</th>
