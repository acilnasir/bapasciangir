...
export default function JabatanSection({ profilId, initialData }: Props) {
  const [data, setData] = useState<Jabatan[]>(initialData);
  ...
  function mulaiEdit(item: Jabatan) {
    setEditId(item.id);
    setNama(item.nama);
    setTahun(item.tahun);
  }
  ...
  async function simpanData() {
    ...
    try {
      ...
      if (editId) {
        const res = await fetch(`/api/jabatan/${editId}`, {
          method: "PUT",
          headers: {
  ...
  async function hapusData(id: string) {
    const confirmDelete = confirm("Yakin ingin menghapus Riwayat Jabatan ini?");
  ...
  return (
    <div className="rounded-xl bg-white p-6 shadow-sm">
      <h2 className="mb-6 text-xl font-bold text-primary">Riwayat Jabatan</h2>
