...
interface Props {
  profilId: string;
  initialData: Penghargaan[];
}

export default function PenghargaanSection({ profilId, initialData }: Props) {
  const [data, setData] = useState<Penghargaan[]>(initialData);
  ...
  function mulaiEdit(item: Penghargaan) {
    setEditId(item.id);
    setNama(item.nama);
    setTahun(item.tahun);
  }
  ...
  async function simpanData() {
    ...
    try {
      ...
      if (editId) {
        const res = await fetch(`/api/penghargaan/${editId}`, {
          method: "PUT",
          headers: {
  ...
  async function hapusData(id: string) {
    const confirmDelete = confirm(
      "Yakin ingin menghapus Riwayat penghargaan ini?",
    );
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-neutral">
              <th className="p-2 text-left">Penghargaan</th>
